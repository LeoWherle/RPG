...
static const sfFloatRect *vil_col_list[] = {
    ...
    &((sfFloatRect){-2, -2, TILE_SIZE * 2, TILE_SIZE}),
    &((sfFloatRect){-1.8, -1.8, TILE_SIZE * 3.9, TILE_SIZE * 1.8}),
    &((sfFloatRect){-2, -2, TILE_SIZE * 2, TILE_SIZE}),
    NULL};

sfFloatRect translate_collisions_village(char c)
{
    char img[] = "_-[]lrLR}{><MtTFfOvG+|bp\\/CVmP@wBx";
...
{
    char img[] = ". -_[]lrLR=/\\}{><MtTFfOvG+|,;:bpCVmPB";
    int i = 0;
...
tile_t **init_village_tile_list(tile_t **tile_list, int map_nb)
{
    ...
    &CAVE_TOWER, &MAN_BIRD, &SHOP, &PALM_TREE, &BOSS_HOUSE, NULL};
